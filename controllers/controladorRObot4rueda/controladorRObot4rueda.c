...
int main(int argc, char **argv) {
  ...
  int onTheAir = 0;
  bool firstTime = 1;
  int alturaMaximaPermitida = 650; // TO-DO ajustar al plano adecuado
  ...
  double bordePistaIzq = 1.44; // Pistas, por si acaso
  double bordePistaDcha = -0.24;
    ...
    if (avoid_obstacle_counter > 0) {
      ...
      if (ds_values[0] < 650.0 || ds_values[1] < 650.0){ // TO-DO aniadir estabilidad, fuzzy logic?
        ...
        if (counterBrujulear <= 0) {
              if (!(coordGPS[1] < bordePistaDcha || coordGPS[1] > bordePistaIzq)) counterBrujulear = 1;
              if (coordIniciales[1] < compasInicialVerdad[1] && coordIniciales[0] > compasInicialVerdad[0]){
